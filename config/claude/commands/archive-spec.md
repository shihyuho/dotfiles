...
description: Move SPEC.md and tasks/* (from agent-skills /spec and /plan) into docs/features/<feature-name>/ so multiple features can coexist without overwriting each other.
---
...
Archive the working artifacts produced by `agent-skills`'s `/spec` and `/plan` commands (`SPEC.md`, `tasks/plan.md`, `tasks/todo.md`) into a feature-scoped directory:
...
- Never delete `SPEC.md`, `tasks/plan.md`, or `tasks/todo.md` without moving them first.
